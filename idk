...
function loader()
    ...
    task.spawn(function()
        ...
        ProgressBar(50, "Bypassing Key system", 1)
        wait(1)
		...
		local function finalizeSetup()
			ProgressBar(20, "Thanks to Blitz team for the Bypassing", 1)
			loadtopbar()
			wait(1)
			ProgressBar(30, "Bypass Complete!", 1)
			wait(0.5)
			MainFrame.LoaderFrame.Visible = false
		...
		local s, ValidateFailed = pcall(function()
			if keyless then
				warn(']---------Trigon Keyless from Blitz team---------[')
				finalizeSetup()
			else
...
trigok.ScriptBox.Text = "-- We recommend only using loadstrings | Trigon Keyless made by Blitz team"
